...
show_accessible = st.sidebar.checkbox("顯示無障礙廁座數", value=True)
...
for _, row in filtered_data.iterrows():
    ...
    popup_info += f"<b>地址:</b> {row['公廁地址']}<br>"
    popup_info += f"<b>管理單位:</b> {row['管理單位']}<br>"
    ...
    m.add_marker(location=(row['緯度'], row['經度']), popup=popup_info)
